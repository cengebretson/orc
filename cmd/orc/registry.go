...
func init() {
	...
	eventsCmd.Flags().BoolVar(&eventsFollow, "follow", false, "Continue watching for workspace changes")
	eventsCmd.Flags().BoolVar(&eventsJSON, "json", false, "Emit newline-delimited JSON")
	eventsCmd.Flags().StringVar(&eventsInterval, "interval", "5s", "Polling interval when following")
	...
	runCmd.Flags().StringVar(&runSlug, "slug", "", "Override the prompt-derived feature slug")
	runCmd.Flags().StringVar(&runWorker, "worker", "", "Worker ID to run the task (prompted when omitted)")
	runCmd.Flags().StringVar(&runRepo, "repo", "", "Target repository name (inferred from the current directory when possible)")
	runCmd.Flags().BoolVar(&runTmux, "tmux", false, "Launch in the selected multiplexer even when settings.auto_tmux is false")
	runCmd.Flags().BoolVar(&runAutoAttach, "attach", false, "Launch in the selected multiplexer and attach immediately")
	_ = runCmd.RegisterFlagCompletionFunc("repo", runRepoCompleter)
	_ = runCmd.RegisterFlagCompletionFunc("worker", runWorkerCompleter)
